...
### Operational state
- **FG-40F NAPALM polling rows manually disabled.** `arp` / `mac` / `lldp` / `bgp` rows for `fg-40f` are `enabled=false` pending Block C (SNMP-collector polling-loop integration). Reversal SQL: `UPDATE device_polls SET enabled = true WHERE device_name = 'fg-40f' AND job_type IN ('arp','mac','lldp','bgp');`
- **arista-mnm NAPALM polling rows manually disabled (2026-04-24).** Same pattern — vEOS lab at 172.21.140.16 doesn't have the NAPALM `eos` driver's eAPI requirement satisfied; NAPALM calls return 503. `arp` / `mac` / `lldp` / `bgp` rows disabled in mnm_controller (4 rows updated). Reversal SQL when Block C ships: `UPDATE device_polls SET enabled = true WHERE device_name = 'arista-mnm' AND job_type IN ('arp','mac','lldp','bgp');`
